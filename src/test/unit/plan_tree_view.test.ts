...
describe('isRelevantPlanMetadata', () => {
  ...
  it('returns true when cwd is a subdirectory of workspace', () => {
    expect(isRelevantPlanMetadata({ cwd: '/test/workspace/subdir' })).toBe(
      true,
    );
  });
  ...
  it('prevents partial path matches', () => {
    expect(isRelevantPlanMetadata({ cwd: '/test/workspace-other' })).toBe(
      false,
    );
  });
});
...
describe('extractPlanTitle', () => {
  ...
  it('extracts generic heading when no Plan: prefix', () => {
    ...
    expect(extractPlanTitle('/plans/random-name.md')).toBe(
      'Some Other Heading',
    );
  });
...
describe('PlanStore', () => {
  ...
  it('sorts by createdAt descending', () => {
    store.add({
      name: 'old.md',
      absPath: '/plans/old.md',
      metadataPath: '/meta/old.json',
      createdAt: '2026-01-01',
      source: 'claude',
      sessionDetected: false,
    });
    store.add({
      name: 'new.md',
      absPath: '/plans/new.md',
      metadataPath: '/meta/new.json',
    ...
    store.add({
      name: 'mid.md',
      absPath: '/plans/mid.md',
      metadataPath: '/meta/mid.json',
      createdAt: '2026-01-02',
      source: 'claude',
      sessionDetected: false,
    });
  ...
  it('add skips entry when plan file does not exist (race condition)', () => {
    ...
    store.add({
      name: 'gone.md',
      absPath: '/plans/gone.md',
      metadataPath: '/meta/gone.json',
      createdAt: '2026-01-01',
      source: 'claude',
      sessionDetected: true,
    });
  ...
  it('scanExisting loads plans from both metadata dirs', () => {
    vi.mocked(fs.readdirSync).mockImplementation((dir) => {
      if (String(dir) === PLAN_METADATA_DIR) {
        return ['claude-plan.json'] as unknown as fs.Dirent[];
      }
      if (String(dir) === GEMINI_PLAN_METADATA_DIR) {
        return ['gemini-plan.json'] as unknown as fs.Dirent[];
      }
    ...
    vi.mocked(fs.readFileSync).mockImplementation((filePath) => {
      if (String(filePath).includes('claude-plan')) {
        return JSON.stringify({
          cwd: '/test/workspace',
          abs_path: '/home/.claude/plans/claude-plan.md',
          created_at: '2026-01-01',
        });
      }
      if (String(filePath).includes('gemini-plan')) {
        return JSON.stringify({
          cwd: '/test/workspace',
          abs_path: '/home/.gemini/plans/gemini-plan.md',
  ...
  it('scanExisting filters out irrelevant plans', () => {
    vi.mocked(fs.readdirSync).mockImplementation((dir) => {
      if (String(dir) === PLAN_METADATA_DIR) {
        return ['other.json'] as unknown as fs.Dirent[];
      }
      return [] as unknown as fs.Dirent[];
    });
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({
        cwd: '/other/project',
        abs_path: '/home/.claude/plans/other.md',
        created_at: '2026-01-01',
      }),
    );
  ...
  it('scanExisting prunes stale plans whose files no longer exist', () => {
    vi.mocked(fs.readdirSync).mockImplementation((dir) => {
      if (String(dir) === PLAN_METADATA_DIR) {
        return ['stale.json'] as unknown as fs.Dirent[];
      }
      return [] as unknown as fs.Dirent[];
    });
    vi.mocked(fs.readFileSync).mockReturnValue(
      JSON.stringify({
        cwd: '/test/workspace',
        abs_path: '/home/.claude/plans/stale.md',
        created_at: '2026-01-01',
      }),
    );
    ...
    expect(fs.unlinkSync).toHaveBeenCalledWith(
      expect.stringContaining('stale.json'),
    );
  });
  ...
  it('scanExisting starts plan file watchers for existing plans', () => {
    vi.mocked(fs.readdirSync).mockImplementation((dir) => {
      if (String(dir) === PLAN_METADATA_DIR) {
        return ['plan.json'] as unknown as fs.Dirent[];
      }
      return [] as unknown as fs.Dirent[];
    });
    vi.mocked(fs.readFileSync).mockImplementation((filePath) => {
      if (String(filePath).includes('plan.json')) {
        return JSON.stringify({
          cwd: '/test/workspace',
          abs_path: '/home/.claude/plans/plan.md',
          created_at: '2026-01-01',
        });
      }
      return '# Plan: Test';
