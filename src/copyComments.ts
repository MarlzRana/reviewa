...
function formatLineContent(comment: ReviewaComment): string {
  const prefix =
    comment.side === 'addition' ? '+' : comment.side === 'removal' ? '-' : '';
...
async function copyFileComments(
  ...
  const matched = store
    .getAll()
    .filter(
      (t) => t.data.status === 'pending' && t.data.abs_path === activeFilePath,
    )
  ...
  vscode.window.showInformationMessage(
    `Reviewa: Copied ${matched.length} comment(s) in current file to clipboard`,
  );
}
...
async function copyAllPendingComments(store: CommentStore): Promise<void> {
  ...
  consumeComments(matched.map((t) => t.data.uuid));
  vscode.window.showInformationMessage(
    `Reviewa: Copied ${matched.length} comment(s) to clipboard`,
  );
}
...
export function registerCopyCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'reviewa.copyFileComments',
      (uri?: vscode.Uri) => copyFileComments(store, uri),
    ),
    vscode.commands.registerCommand('reviewa.copyAllPendingComments', () =>
      copyAllPendingComments(store),
    ),
    vscode.commands.registerCommand(
      'reviewa.copyFileCommentsClaudeCode',
      (uri?: vscode.Uri) => copyFileComments(store, uri),
    ),
    vscode.commands.registerCommand(
      'reviewa.copyAllPendingCommentsClaudeCode',
      () => copyAllPendingComments(store),
    ),
  );
}
